...
2. **`modality.json`** — Mirrors the LeRobot `meta/modality.json` file produced after converting your dataset. Adjust the start/end indices and modality names to reflect the true layout exported by your data pipeline. Make sure the language key (default: `annotation.language.task`) matches the key emitted by your annotation exporter.
