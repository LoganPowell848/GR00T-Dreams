...
2. Edit the files under [`IDM_dump/global_metadata/nps_hamming`](../IDM_dump/global_metadata/nps_hamming):
   - Update `modality.json` so the `start`/`end` indices match the flattened observation and action vectors exported by your simulator or robot logs. Ensure that the language entry (`annotation.language.task` by default) mirrors the key written in your dataset metadata.
...
The converter now automatically maps the annotation channel to `annotation.language.task`. If you store instructions under a different key, override it with `--annotation_source` (see `python IDM_dump/raw_to_lerobot.py --help`).
